...
class NeuromorphicOS(nn.Module):
    ...
    def __init__(self, config: Dict[str, Any], device_name: Optional[str] = "auto"):
        ...
        logger.info(f"🖥️  Neuromorphic OS booting on: {self.hardware.device_name}")
        ...
        # 3. Cognitive Modules
        self.global_workspace = GlobalWorkspace(dim=self.config.get("dim", 64)).to(self.hardware.device)
    ...
    def _build_research_substrate(self) -> None:
        ...
        self.substrate.add_projection("assoc_to_v1", "Association", "V1", plasticity_rule=active_inf)
    ...
    def run_cycle(self, sensory_input: torch.Tensor, phase: str = "wake") -> Dict[str, Any]:
        ...
        if phase == "wake":
            ...
            # --- SLEEP MODE ---
            maint_stats = self.sleep_manager.perform_maintenance(self.cycle_count)
        ...
        target = current_input if phase == "wake" else (substrate_inputs.get("V1") if "V1" in substrate_inputs else None)
    ...
    def _pack_observation(self, phase, learning_phase, state, conscious_lvl, logs):
        """観測データのパッケージング"""
        activity = {k: float(v.mean().item()) for k, v in state["spikes"].items() if v is not None}
