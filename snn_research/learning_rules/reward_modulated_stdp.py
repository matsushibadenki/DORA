...
class RewardModulatedSTDP(BioLearningRule):
    ...
    def update(
        ...
        **kwargs: Any
        ...
        # 報酬の取得 (local_state または kwargs から)
        reward = 0.0
        if local_state is not None and 'reward' in local_state:
            reward = local_state['reward']
        ...
        if self.eligibility_trace is None:
            self.eligibility_trace = torch.zeros_like(current_weights)
...
class EmotionModulatedSTDP(RewardModulatedSTDP):
    def update(
        ...
        **kwargs: Any
        ...
        if local_state:
            external_reward = local_state.get('external_reward', 0.0)
            internal_value = local_state.get('internal_value', 0.0)
        else:
