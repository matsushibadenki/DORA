...
class BCMLearningRule(BioLearningRule):
    ...
    def __init__(
        ...
        print(
            f"🧠 BCM V16.5 initialized (Target: {target_rate}, High Stability Mode)")
    ...
    def update(
        ...
        **kwargs: Any
        ...
        # バッチ平均の取得
        pre_avg = pre_spikes.mean(
            dim=0) if pre_spikes.dim() > 1 else pre_spikes
        post_avg = post_spikes.mean(
            dim=0) if post_spikes.dim() > 1 else post_spikes
        ...
        with torch.no_grad():
            new_avg = (1.0 - self.avg_decay_factor) * \
                avg_act + self.avg_decay_factor * post_avg
