...
class HierarchicalMemorySystem:
    ...
    def store_experience(self, experience: Dict, spike_activity: np.ndarray):
        ...
        if importance > 0.8:
            # 即座に長期記憶へ（高重要度）
            logger.info(
                f"⚡ Instant consolidation for high importance memory: {key[:8]}...")
            self._consolidate_to_cortex(trace)
        elif importance > 0.4:
            # 海馬（DB）で保持（中重要度）
            self.hippocampus_db[key] = trace
            # 既存のHippocampusモジュールとも同期（オプション）
            # store_episodeはTensorのみ受け付けるため、processメソッドを使用
            self.hippocampus_module.process(experience)
        else:
            # エピソード記憶へ（低重要度、一時的）
    ...
    def sleep_consolidation(self, duration_steps: int = 1000):
        ...
        logger.info(
            f"💤 Sleep consolidation starting ({duration_steps} steps)...")
        ...
        logger.info(
            f"✅ Consolidation complete. Transferred: {transferred_count}")
    ...
    def _forget_low_importance_memories(self):
        ...
        for key, trace in self.hippocampus_db.items():
            ...
            # 重要度が減衰する
            decayed_importance = trace.importance * \
                np.exp(-trace.decay_rate * (elapsed / 3600.0))  # 1時間単位
        ...
        if keys_to_delete:
            logger.info(
                f"🧹 Forgotten {len(keys_to_delete)} low-importance memories.")
