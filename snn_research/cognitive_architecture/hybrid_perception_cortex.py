...
class HybridPerceptionCortex(nn.Module):
    ...
    def __init__(
        ...
        if stdp_params is None:
            stdp_params = {'learning_rate': 0.005,
                           'a_plus': 1.0, 'a_minus': 1.0, 'tau_trace': 20.0}
    ...
    def perceive(self, sensory_input: torch.Tensor) -> Dict[str, Any]:
        ...
        # 状態更新
        self.prev_column_state = {k: v.detach()
        ...
        # 活性度計算
        column_activity = sum(t.mean().item()
                              for t in current_states.values()) / len(current_states)
    ...
    def perceive_and_upload(self, spike_pattern: torch.Tensor) -> None:
        ...
        print(
            f"  - 知覚野: 皮質カラム処理完了 (活性度: {result['column_activity']:.2f}) -> Workspaceへ送信")
