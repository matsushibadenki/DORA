...
import torch  # Added for torch.randn and tensor operations
...
class EmergentCognitiveSystem:
    ...
    async def run_cooperative_observation_task(self):
        ...
        if not observer_agent or not recipient_agent:
            print(
                "❌ Task failed: Required agents ('AutonomousAgent', 'SpecialistAgent') not found.")
            return

        # 1. 観測者エージェントが情報を発見し、スパイクにエンコード
        observation = {"intent": "share_finding",
                       "payload": "Found a new pattern of high activity in sector 7."}
        print(
            f"👀 Observer '{observer_agent.name}' makes an observation: {observation}")
        spike_message = observer_agent.spike_communicator.encode_data(
            observation)

        # 2. 観測者がGlobalWorkspaceに情報をアップロード
        print(
            f"📢 Observer '{observer_agent.name}' uploads the finding to the Global Workspace.")
        # ◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️↓修正開始◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️
        self.global_workspace.upload_to_workspace(
            source_name=observer_agent.name,
            content=observation,
        ...
        recipient_agent.receive_and_process_spike_message(
            spike_message, source_agent=observer_agent.name)
    ...
    async def _find_collaborator_for_task(self, failed_task: Dict[str, Any], failed_agent: 'AutonomousAgent') -> Optional[Tuple[str, Dict[str, Any]]]:
        ...
        original_performance = original_expert_info.get("metrics", {}).get(
            "accuracy", 0.0) if original_expert_info else 0.0
        ...
        if best_collaborator and best_new_task:
            print(
                f"✅ Collaborator found: Agent '{best_collaborator}' has a better model ('{best_new_task['expert_id']}') with performance {best_performance:.4f}.")
    ...
    async def execute_task_async(self, high_level_goal: str) -> str:
        ...
        self.global_workspace.upload_to_workspace(
            source_name="planner",
            content={"features": torch.randn(
                1, 64), "message": f"New plan created: {plan.task_list}"},
            salience=0.7
        )
        # ◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️↑修正終わり◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️
        ...
        while task_queue:
            ...
            print(
                f"-> Assigning task '{task_description}' to agent '{agent.name}'")
            ...
            if is_success:
                expert_id = execution_result.get(
                    'model_id', 'unknown') if execution_result else 'unknown'
                result = f"SUCCESS: Task '{task_description}' completed by '{agent.name}' using expert '{expert_id}'."
                results.append(result)
                # ◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️↓修正開始◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️
                self.global_workspace.upload_to_workspace(
                    agent.name, {"content": result}, salience=0.6)
                # ◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️↑修正終わり◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️
            else:
                ...
                # ◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️↓修正開始◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️
                self.global_workspace.upload_to_workspace(
                    agent.name, {"content": result}, salience=0.9)  # 失敗は顕著性が高い
                # ◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️↑修正終わり◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️
                ...
                if collaboration_proposal:
                    ...
                    print(
                        f"++ Collaboration proposed! Re-assigning task to agent '{collaborator_name}'.")
        ...
        self.global_workspace.upload_to_workspace(
            source_name="system",
            content={"features": torch.randn(
                1, 64), "message": f"Goal '{high_level_goal}' completed. Final report generated."},
            salience=0.7
        )
        # ◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️↑修正終わり◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️◾️
