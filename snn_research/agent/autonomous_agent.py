...
class AutonomousAgent:
    ...
    def __init__(self, input_size: int, output_size: int, device: str, config_path: Optional[str] = None, name: str = "AutonomousAgent"):
        ...
        if config_path and os.path.exists(config_path):
            try:
                with open(config_path, 'r') as f:
                    ...
                    print(
                        f"  ⚙️ [Autonomy] Loaded scaled config from {config_path}")
            except Exception as e:
                print(
                    f"  ⚠️ [Autonomy] Config load failed: {e}. Using default.")
        ...
        self.encoder = UniversalEncoder(
            output_dim=model_config.get('d_model', 64), device=device)
    ...
    def perceive_and_act(self, state: torch.Tensor, step: int) -> int:
        ...
        if confidence < self.curiosity_threshold:
            print(
                f"  [Autonomy] Unknown state detected (Conf: {confidence:.2f}). Triggering curiosity...")
    ...
    def load(self, path: str):
        self.brain.model.load_state_dict(
