...
class TestUniversalEncoder(unittest.TestCase):
    ...
    def test_image_rate_coding(self):
        ...
        expected_features = self.encoder.output_dim  # 784 for dummy
        self.assertEqual(spikes.shape, (self.batch_size,
                         self.time_steps, expected_features))
    ...
    def test_image_latency_coding(self):
        ...
        # Use output_dim as features for test compatibility with dummy
        features = self.encoder.output_dim
        image_flat = torch.rand(self.batch_size, features)

        spikes = self.encoder.encode(
            image_flat, modality='image', method='latency')
    ...
    def test_audio_delta_coding(self):
        ...
        input_time = 20
        input_time = 20
        features = self.encoder.output_dim  # 64 -> 784
        waveform = torch.sin(torch.linspace(0, 10, input_time)).unsqueeze(
            0).unsqueeze(2).expand(self.batch_size, -1, features)

        spikes = self.encoder.encode(
            waveform, modality='audio', method='delta')
    ...
    def test_dvs_passthrough(self):
        ...
        input_time = 15
        C, H, W = 2, 34, 34
        dvs_data = (torch.rand(self.batch_size,
                    input_time, C, H, W) > 0.9).float()

        spikes = self.encoder.encode(dvs_data, modality='dvs')

        expected_features = self.encoder.output_dim
