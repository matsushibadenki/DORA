...
class TestLiquidAssociation(unittest.TestCase):
    ...
    def test_multimodal_integration(self):
        ...
        raw_audio = torch.rand(self.batch_size, 20,
                               self.dim_audio)  # (B, Time, Feat)
        raw_text = torch.randn(self.batch_size, self.dim_text)      # Embedding

        # 2. スパイクエンコード (Batch, Time, Features)
        spike_visual = self.encoder.encode(
            raw_image, modality='image', method='rate')
        spike_audio = self.encoder.encode(
            raw_audio, modality='audio', method='delta')
        spike_text = self.encoder.encode(
            raw_text, modality='text', method='rate')
        ...
        self.assertGreater(
            total_activity, 0, "Reservoir remained silent despite multimodal input.")

        # 形状確認
        self.assertEqual(reservoir_out.shape,
                         (self.batch_size, self.reservoir_size))
