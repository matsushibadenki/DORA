...
def create_agent(name: str, device: str, noise_level: float = 0.0) -> Tuple[SynestheticAgent, TheoryOfMindModule, float]:
    ...
    wm_module = ArchitectureRegistry.build(
    ...
    tom = TheoryOfMindModule(
        observation_dim=4, hidden_dim=16, history_len=5).to(device)
...
def main():
    ...
    if avg_acc > 0.6:
        logger.info(
            "✅ Collective Intelligence Emerged! The group performed better than random.")
    else:
        logger.info(
            "⚠️ Performance Low. Agents need more training to trust experts.")
