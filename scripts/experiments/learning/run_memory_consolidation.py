...
class PrototypeReadout:
    ...
    def update(self, features, label):
        ...
        if features.shape[0] != self.prototypes.shape[1]: return
        alpha = 0.1
    ...
    def predict(self, features):
        if self.counts.sum() == 0: return -1 
        features = features.cpu().view(1, -1)
        f_norm = features / (features.norm() + 1e-8)
        p_norm = self.prototypes / (self.prototypes.norm(dim=1, keepdim=True) + 1e-8)
...
def run_memory_experiment():
    ...
    readout = PrototypeReadout(num_classes=10, feature_dim=config["hidden_dim"])
    data_loader = load_mnist_sample(batch_size=1)
    data_iterator = iter(data_loader)
    
    schedule = [
        {"phase": "wake",  "end_cycle": 300, "label": "Learning Phase 1"},
        {"phase": "sleep", "end_cycle": 450, "label": "Memory Consolidation (Pruning/Growth)"},
        {"phase": "wake",  "end_cycle": 800, "label": "Learning Phase 2 (Recall)"}
    ]
    ...
    try:
        while cycle_counter < total_cycles:
            ...
            # --- Run OS ---
            observation = os_kernel.run_cycle(sensory_input, phase=current_phase)
            ...
            if raw_spikes is None:
                raw_spikes = torch.zeros(1, config["hidden_dim"]).to(os_kernel.device)
            else:
            ...
            # --- Recording ---
            acc_val = sum(recent_accuracy)/len(recent_accuracy) if recent_accuracy else 0.0
            
            record = {
                "cycle": cycle_counter,
                "phase": current_phase,
                "phase_label": phase_label,
                "energy": observation["bio_metrics"]["current_energy"],
        ...
        out_path = os.path.join("runtime_state", "memory_experiment_history.json")
        try:
            with open(out_path, "w") as f:
                json.dump(history, f, indent=2)
            print(f"💾 Learning history saved to: {out_path}")
            print("📊 Run 'python scripts/visualization/plot_memory_learning.py' to visualize.")
        except Exception:
